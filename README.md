...
6. Tunes each model with Spark CrossValidator and 6 grid-search combinations
7. Selects a threshold on validation-user target rows
8. Adds sampled unobserved tracks to the test set as artificial ranking negatives
9. Reports PR-AUC, binary threshold metrics, and user-level ranking metrics
10. Saves models, train/test data, predictions, soft scores, and evaluation output
...
- Test rows include up to 30 sampled unobserved tracks per test user with
  `label=0`; these are artificial ranking negatives, not explicit dislikes
- History features are bounded last-50 statistical aggregates
- Final test metrics are reported on held-out target rows plus sampled
  unobserved candidates
- Ranking metrics are reported at `10`
